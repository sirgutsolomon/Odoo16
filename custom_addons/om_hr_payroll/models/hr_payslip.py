    ...
    def _compute_absent_days(self):
        for payslip in self:
            if payslip.employee_id and payslip.date_from and payslip.date_to:
                # Search for approved leaves within the payslip date range
                employee_leaves = self.env['hr.leave'].search([
                    ('employee_id', '=', payslip.employee_id.id),
                    ('date_from', '>=', payslip.date_from),
                    ('date_to', '<=', payslip.date_to),
                    ('state', '=', 'validate')  # Only consider approved leaves
                ])

                # Calculate total absent days
                total_absent_days = sum(leave.number_of_days for leave in employee_leaves)
                payslip.absent_days = total_absent_days
            else:
        ...
        for contract in contracts:
            ...
            for rule in sorted_rules:
                ...
                if rule._satisfy_condition(localdict) and rule.id not in blacklist:
                    ...
                    localdict['allowances'] = contract.allowance_ids
                    localdict['absent_days'] = absent_day
                    localdict['salary_rules'] = sorted_rules
